...
namespace PCAB_Debugger_ACS
{
    public class PCABs
    {
        ...
        public List<cntConfigPorts> PCAB_Boards {  get; private set; } = new List<cntConfigPorts>();
        ...
        public PCABs(string SerialPortName, UInt32 BaudRate)
        {
        ...
        public void Open(List<SN_POSI> SerialNumber_Posi, uint MonitorIntervalTime)
        {
            ...
            if (result && ret.Count == SerialNumber_Posi.Count)
            {
                ...
                for (int snCNT = 0; snCNT < SerialNumber_Posi.Count; snCNT++)
                {
                    if (ret[snCNT])
                    {
                        PCAB_Boards.Add(new cntConfigPorts(_task.UNITs[cnt].SerialNumberASCII, SerialNumber_Posi[snCNT].RotateCODE));
