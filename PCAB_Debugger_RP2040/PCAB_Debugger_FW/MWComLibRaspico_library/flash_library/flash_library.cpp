...
bool flash::moveROM(const uint32_t &targetAddr, const uint32_t &sourceAddr, const uint32_t &count)
{
    if(count % FLASH_SECTOR_SIZE != 0) { return false; }
    if((targetAddr & (FLASH_SECTOR_SIZE - 1)) != 0 || (sourceAddr & (FLASH_SECTOR_SIZE - 1)) != 0) { return false; }
    if(PICO_FLASH_SIZE_BYTES < targetAddr + count || PICO_FLASH_SIZE_BYTES < sourceAddr + count) { return false; }
    uint32_t ints = save_and_disable_interrupts();

    for(uint32_t i = 0; i < count / FLASH_SECTOR_SIZE; i++)
    {
        flash_range_erase(targetAddr + i * FLASH_SECTOR_SIZE, FLASH_SECTOR_SIZE);
        for(uint32_t j = 0; j < FLASH_SECTOR_SIZE / FLASH_PAGE_SIZE ; j++)
        {
            const uint8_t *flash_target_contents = (const uint8_t *) (XIP_BASE + sourceAddr + i * FLASH_SECTOR_SIZE + j * FLASH_PAGE_SIZE);
            uint8_t pageDAT[FLASH_PAGE_SIZE];
            for(uint i = 0 ; i < FLASH_PAGE_SIZE ; i++){pageDAT[i] = flash_target_contents[i];}
            flash_range_program(targetAddr + i * FLASH_SECTOR_SIZE + j * FLASH_PAGE_SIZE, pageDAT, FLASH_PAGE_SIZE);
        }
        flash_range_erase(sourceAddr + i * FLASH_SECTOR_SIZE, FLASH_SECTOR_SIZE);
    }
    restore_interrupts(ints);
    return true;
}

bool flash::updateROM(const uint32_t &targetAddr, const uint32_t &sourceAddr, const uint32_t &count)
{
    if(count % FLASH_SECTOR_SIZE != 0) { return false; }
    if((targetAddr & (FLASH_SECTOR_SIZE - 1)) != 0 || (sourceAddr & (FLASH_SECTOR_SIZE - 1)) != 0) { return false; }
    if(PICO_FLASH_SIZE_BYTES < targetAddr + count || PICO_FLASH_SIZE_BYTES < sourceAddr + count) { return false; }
